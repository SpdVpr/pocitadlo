...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      console.log('[AUTH_CONTEXT] Auth state changed, user:', currentUser?.uid, 'encryptionKey:', !!encryptionKey);
      setUser(currentUser);
      ...
      if (!currentUser) {
        ...
        console.log('[AUTH_CONTEXT] User logged in, storedKeyType:', storedKeyType, 'hasEncryptionKey:', !!encryptionKey);
        ...
        if (!encryptionKey) {
          ...
          console.log('[AUTH_CONTEXT] Encryption key already exists, skipping derivation');
        }
