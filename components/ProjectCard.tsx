...
export default function ProjectCard({
  ...
  // Previous months' unfactured time = total accumulator minus current month
  const previousMonthsTime = Math.max(0, project.totalTimeCurrentMonth - currentMonthTime);
  const previousMonthsPrice = Math.max(0, project.totalPriceCurrentMonth - currentMonthPrice);
  return (
    <motion.div
      onClick={onSelect}
      ...
      <div className="relative flex items-center justify-between mb-2 sm:mb-3">
        ...
        >
          <p className="text-xl sm:text-2xl font-bold text-green-600">
            {formatPrice(currentMonthPrice, project.currency || 'CZK')}
          </p>
          <p className="text-xs sm:text-sm text-gray-500">tento měsíc</p>
          {previousMonthsPrice > 0 && (
            <p className="text-[10px] sm:text-xs text-orange-500 mt-0.5">
              + {formatPrice(previousMonthsPrice, project.currency || 'CZK')} nevyfakturováno
            </p>
          )}
        </motion.div>
      </div>
